...
async function editarContaView(req, res) {
  ...
  ContaCorrente.findByPk(req.session.id_conta, {
    include: [
      {
        model: Usuario,
        include: [Pessoa],
...
async function editarConta(req, res) {
  const pessoa = await Pessoa.findOne({ where: { cpf: req.body.cpf } });

  if (!pessoa) {
    return res.render("contaCorrente/editar.html", {
      erro: "Não foi possível nenhum usuário com o CPF informado",
    });
  }

  const usuario = await Usuario.findOne({ where: { pessoaId: pessoa.id } });
  ...
  ContaCorrente.update(contaCorrente, { where: { id: req.session.id_conta } })
...
function excluirContaView(req, res) {
  req.session.id_conta = req.params.id;
  ContaCorrente.findByPk(req.session.id_conta).then(function (conta) {
...
function depositoView(req, res) {
  req.session.id_conta = req.params.id;
  ContaCorrente.findByPk(req.session.id_conta).then(function (conta) {
    res.render("contaCorrente/deposito.html", { conta });
  });
}
...
async function realizarDeposito(req, res) {
  ...
  const conta = await ContaCorrente.findByPk(req.session.id_conta);
...
function extratoView(req, res) {
  req.session.id_conta = req.params.id;
  Movimento.findAll({ where: { contaCorrenteId: req.session.id_conta } }).then(function (
    movimentos
  ) {
